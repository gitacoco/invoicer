  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-[440px] max-h-[90vh] overflow-y-auto p-6 flex flex-col gap-5">
        <h3 className="text-base font-bold text-dark">Toggl Track Integration</h3>

        {/* Enable toggle switch */}
        <label className="flex items-center justify-between cursor-pointer">
          <span className="text-sm text-dark">Enable Toggl Track</span>
          <button
            type="button"
            role="switch"
            ...
            className={`relative inline-flex h-6 w-11 shrink-0 rounded-full transition-colors ${
              config.enabled ? "bg-brand" : "bg-gray-200"
            }`}
          >
            <span
...
export function TogglSyncPrompt({
  ...
  return (
    <div className="flex flex-col gap-2 bg-purple-50 rounded-lg p-3">
      ...
      {fetching ? (
        <p className="text-[12px] text-purple-600">Fetching entries...</p>
      ) : !hasFetched ? (
        <button
          className="text-[12px] text-purple-600 hover:text-purple-800 transition-colors text-left"
          onClick={onSync}
        >
        ...
        <>
          <p className="text-[12px] text-purple-700">
            You have <strong>{pendingEntries.length}</strong> new{" "}
            {pendingEntries.length === 1 ? "entry" : "entries"} in Toggl Track.
          </p>
          <div className="flex flex-col gap-1 max-h-[120px] overflow-y-auto">
            {pendingEntries.map((e) => (
              <div
                key={e.date}
                className="flex justify-between text-[11px] text-purple-600 bg-white/60 rounded px-2 py-1"
              >
                <span>{e.date}</span>
                <span>{e.hours}h</span>
              </div>
          ...
          <button
            className="bg-purple-600 text-white rounded-lg px-3 py-2 text-sm font-semibold hover:bg-purple-700 transition-colors"
            onClick={onImport}
          >
            Import {pendingEntries.length}{" "}
